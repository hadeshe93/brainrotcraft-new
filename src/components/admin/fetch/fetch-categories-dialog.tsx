...
interface FetchCategoriesDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export default function FetchCategoriesDialog({ open, onOpenChange }: FetchCategoriesDialogProps) {
  ...
  const loadCategories = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to fetch categories from parent site');
      }

      const result: { success: boolean; data: Category[]; error?: string } = await response.json();
      ...
      // 获取本地已存在的分类
      const localResponse = await fetch('/api/admin/categories?pageSize=100');
      const localData: { data: Category[] } = await localResponse.json();
      const localUuids = new Set(localData.data.map((cat) => cat.uuid));

      // 过滤掉已存在的
      const newCategories = result.data.filter((cat: Category) => !localUuids.has(cat.uuid));

      setCategories(newCategories);

      if (newCategories.length === 0) {
        toast.info('所有分类已存在', {
          description: '没有新的分类需要导入',
        });
      }
    } catch (error: any) {
  ...
  const handlePreviewAndReview = async () => {
    ...
    const uuids = categories.map((cat) => cat.uuid);
    setLoading(true);
  ...
  // 预览单个
  const handlePreviewSingle = async (category: Category) => {
    setLoading(true);
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="flex max-h-[80vh] max-w-4xl flex-col overflow-hidden">
          <DialogHeader>
            ...
            <DialogDescription>从母站点拉取分类数据，仅显示本地不存在的分类</DialogDescription>
          </DialogHeader>
          ...
          <div className="flex items-center justify-between py-2">
            <div className="text-muted-foreground text-sm">
              {loading ? (
                ...
                <span>显示 {categories.length} 条未导入数据</span>
              )}
            </div>

            <Button onClick={handlePreviewAndReview} disabled={loading || categories.length === 0} size="sm">
              {loading ? (
                      ...
                      <TableCell className="text-right">
                        {imported.has(category.uuid) ? (
                          ...
                          <Button
                            variant="default"
                            size="sm"
                            onClick={() => handlePreviewSingle(category)}
                            disabled={loading}
                          >
                            {loading ? (
