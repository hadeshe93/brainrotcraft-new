...
import { getAllTags, getTagsSEOContent } from '@/services/content/list';
...
async function TagsPage(props: LocalePageProps) {
  const { locale } = await props.params;
  const t = await getTranslations('biz.tag');
  ...
  const [tags, seoContent, sidebar] = await Promise.all([
    getAllTags(db, locale),
    getTagsSEOContent(db, locale),
    ...
    <div className="block-section">
      <div className="flex flex-col gap-6 md:flex-row">
        ...
        <main className="flex-1 space-y-8">
          {/* Page Title */}
          <div>
            <h1 className="text-foreground text-3xl font-bold md:text-4xl">{t('all_tags')}</h1>
            <p className="text-muted-foreground mt-2 text-sm">{t('browse_all', { count: tags.length })}</p>
          </div>

          {/* Tags Grid */}
          <LinkGrid items={tags} baseUrl="/tag" />
...
export async function generateMetadata(props: LocalePageProps) {
  ...
  const env = await getCloudflareEnv();
  const db = (env as any).DB as D1Database;
  const seoContent = await getTagsSEOContent(db, locale);
...
export default wrapForI18n<LocalePageProps>(TagsPage);
