...
async function Privacy(props: LocalePageProps) {
  ...
  const about = await getPageContent<PageKey>({ key: PAGE_KEY, locale });

  return (
    <article className="block-section max-w-4xl">
      {checkShow(about!.breadcrumb) && <BreadcrumbComponent config={about!.breadcrumb!} />}
      <MarkdownRenderer
        locale={locale}
        content={about!.content.introduction}
